...
function tt_img($atts, $content = null)
{
    ...
    if (!empty($link)) {
        $retval = '<a href="' . $link . '" target="' . $target . '">' . $retval . '</a>';
    }
...
add_shortcode('tt_posts', 'tt_posts');
// echo do_shortcode('[tt_posts limit="-1" cat_name="home"]');
function tt_posts($atts)
{
    ...
    if (array_key_exists('photo', $user_data)) {
    ...
    if ($the_query->have_posts()) {
        while ($the_query->have_posts()) {
            ...
            $size = '250,125';
            $image = get_the_post_thumbnail($post_id, $size, $attr = null);
            if (empty($image)) {
